...
cp .env.example .env
```
.env.example 파일을 .env 파일로 복사하고 필요한 환경 변수를 채워주세요.

`./secrets` 디렉토리에 backup 서비스에서 페이지 덤프 백업용으로 사용되는 backup-pages-github-app.pem 파일이 존재해야 합니다.

### 개발 환경

`just set-dev` `just clear-dev`로 개발 환경을 전환하세요.
...
개발 환경에서는 [php-dev.ini](dev/php-dev.ini)가 마운트되며 해당 파일 안에 opcache.revalidate_freq=0이 설정되어 있어 변경사항이 항상 즉시 반영되므로 필요하지 않습니다.
...
### database
데이터베이스 서비스입니다. 미디어위키 권장인 MariaDB를 사용합니다. 창팝위키는 Cargo 확장용 데이터베이스를 별도로 사용하므로, 초기 실행 시 두 개의 DB를 생성하기 위해 별도로 [init-db.sh](build-context/database/init-db.sh) 파일을 사용합니다
...
| `just set-dev`                                                                                                                 | `set-dev`             | 개발 환경 설정 (compose.override.yaml, DebugSettings.php 생성). 기존 파일은 덮어쓰지 않습니다.                                                                                                    |
| `just clear-dev`                                                                                                               | `clear-dev`           | 개발 환경 해제 (compose.override.yaml, DebugSettings.php 삭제).                                                                                                                                 |